...
abstract class Action {
   ...
   /**
     ...
     * @return void
     +----------------------------------------------------------
     */
    protected function error($message,$jumpUrl='',$ajax=false,$adddata=array()) {
        $this->dispatchJump($message,0,$jumpUrl,$ajax,$adddata);
    }
     ...
     * @return void
     +----------------------------------------------------------
     */
    protected function success($message,$jumpUrl='',$ajax=false,$adddata=array()) {
        $this->dispatchJump($message,1,$jumpUrl,$ajax,$adddata);
    }
     ...
     * @return void
     +----------------------------------------------------------
     */
    protected function ajaxReturn($data,$info='',$status=1,$type='',$adddata=array()) {
        $result  =  array();
     ...
     * @return void
     +----------------------------------------------------------
     */
    private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false,$adddata = array()) {
        // 判断是否为AJAX返回
        if($ajax || $this->isAjax()) $this->ajaxReturn($ajax,$message,$status,$adddata);
