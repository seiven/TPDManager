...
class manager_roleModel extends CommonModel {
	...
	function setGroupApps($groupId,$appIdList)
	{
		...
		$result = $this->db->execute('INSERT INTO '.C('RBAC_ACCESS_TABLE').' (role_id,node_id,pid,level) SELECT a.id, b.id,b.pid,b.level FROM '.C('RBAC_ROLE_TABLE').' a, '.C('RBAC_NODE_TABLE').' b WHERE '.$where);
		if($result===false) {
			return false;
		}else {
			return true;
	...
	{
		$rs = $this->db->query('select b.id,b.title,b.name from '.C('RBAC_ACCESS_TABLE').' as a ,'.C('RBAC_NODE_TABLE').' as b where a.node_id=b.id and  b.pid=0 and a.role_id='.$groupId.' ');
		return $rs;
	}
	...
	{
		$table = C('RBAC_ACCESS_TABLE');
		$rs = $this->db->query('select b.id,b.title,b.name from '.$table.' as a ,'.C('RBAC_NODE_TABLE').' as b where a.node_id=b.id and  b.pid='.$appId.' and a.role_id='.$groupId.' ');
		return $rs;
	}
...
function setGroupActions($groupId,$actionIdList)
{
	...
	function getGroupUserList($groupId)
	{
		...
		$rs = $this->db->query('select b.id,b.nickname,b.email from '.$table.' as a ,'.C('RBAC_MUSER_TABLE').' as b where a.user_id=b.id and  a.role_id='.$groupId.' ');
		return $rs;
	}

	function delGroupUser($groupId)
	{
		$table = C('RBAC_USER_TABLE');
	...
	function setGroupUser($groupId,$userId) {
		$sql	=	"INSERT INTO ".C('RBAC_USER_TABLE').' (role_id,user_id) values ('.$groupId.','.$userId.')';
	...
	function setGroupUsers($groupId,$userIdList)
	{
		...
		$rs = $this->execute('INSERT INTO '.C('RBAC_USER_TABLE').' (role_id,user_id) SELECT a.id, b.id FROM '.C('RBAC_ROLE_TABLE').' a, '.C('RBAC_MUSER_TABLE').' b WHERE '.$where);
		if($result===false) {
			return false;
		}else {
			return true;
